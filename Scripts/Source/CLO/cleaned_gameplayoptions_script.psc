Scriptname CLO:Cleaned_GameplayOptions_Script extends Quest
{This is the SQ_PEO_QuestScript from Bethesda but with the XP options mult global and ENV gameplay option properties removed along with the 2 small sets of code they used. 
There's really no need for the ENV gameplay option remap to exist, just change it back normally.
I don't use the XP mult values for gameplay options because I don't believe in punishing the player for playing how they like. XP gain isn't a real tradeoff if it's whole gameplay systems you're changing and not character builds.}
